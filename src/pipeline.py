...
class StableSyncMVDPipeline(StableDiffusionControlNetPipeline):
    ...
    def __call__(
            ...
            bottom_cameras=True,
            texture_size=2048,
            render_rgb_size=2048,
            texture_rgb_size=2048,
                ...
                for prompt_tag, prompt_embeds in prompt_embeds_groups.items():
                    ...
                    for model_input_batch, prompt_embeds_batch, down_block_res_samples_batch, mid_block_res_sample_batch, meta \
                            in zip(model_input_batches, prompt_embeds_batches, down_block_res_samples_list,
                                   mid_block_res_sample_list, self.group_metas):
                ...
                if i % log_interval == log_interval - 1 or t == 1:
                    ...
                    if not t < (1 - multiview_diffusion_end) * num_timesteps:
                        if tex_fast_preview:
                            ...
                            result_tex_rgb, result_tex_rgb_output = get_rgb_texture(self.vae, self.uvp_rgb,
        ...
        if export_glb:
            ...
            mesh = trimesh.load(f"{self.result_dir}/textured.obj")

            # Assign the texture to the mesh
            pbr_material = trimesh.visual.material.SimpleMaterial(image=Image.fromarray(texture_image)).to_pbr()
            mesh.visual.texture = trimesh.visual.TextureVisuals(material=pbr_material)
            mesh.visual.material = mesh.visual.material.to_pbr()
            mesh.visual.material.baseColorFactor = [1.0, 1.0, 1.0]
